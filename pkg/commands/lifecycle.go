...
func newLifecycleCommand(use, short string, run func(ctx context.Context, svc *managers.Services, name string, opts managers.LifecycleOptions) (*managers.LifecycleResult, error)) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			result, err := run(cmd.Context(), svc, args[0], managers.LifecycleOptions{Force: flagForce, DryRun: flagDryRun})
			if err != nil {
				return err
			}
			if flagJSON {
...
var archiveCmd = newLifecycleCommand("archive NAME", "Move an active entry into the archive",
	func(ctx context.Context, s *managers.Services, name string, o managers.LifecycleOptions) (*managers.LifecycleResult, error) {
