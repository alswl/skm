...
func (s *Services) prepareRepo(ctx context.Context, opts DeployOptions) (string, string, error) {
	...
	if isDeployGitURL(repo) || (dal.PathExists(repo) && isBareRepo(repo)) {
...
func (s *Services) cloneOrPull(ctx context.Context, url, dir string) (string, error) {
	if dal.PathExists(dir) {
		...
		if remote := gitRemoteURL(dir); remote != "" && remote != normalizeDeployGitURL(url) {
			return "", common.WithExitCode(fmt.Errorf("deploy: existing clone %s has remote %q, expected %q", dir, remote, normalizeDeployGitURL(url)), common.ExitError)
		}
...
func isDeployGitURL(s string) bool {
...
func normalizeDeployGitURL(s string) string {
...
func gitClone(ctx context.Context, url, dir string) error {
	...
	cmd := exec.CommandContext(ctx, "git", "clone", "--depth", "1", normalizeDeployGitURL(url), dir)
