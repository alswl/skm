...
func TestExternalTargetPluginDiffAndDangling(t *testing.T) {
	dir := t.TempDir()
...
`
	...
	diff, err := p.Diff(t.Context(), entry, target)
	require.NoError(t, err)
	require.Contains(t, diff, "external")
	require.Contains(t, diff, "managed")
...
`
	...
	inst := installer.NewInstaller([]common.InstallTarget{target}, map[string]installer.TargetDriver{"dispatch": externalTargetDriver{plugin}})
	diff, err := inst.Diff(context.Background(), entry, target)
	require.NoError(t, err)
