...
func (p *TargetPlugin) call(ctx context.Context, req targetPluginRequest) (*targetPluginResponse, error) {
	...
	if err != nil {
		if errors.Is(ctx.Err(), context.DeadlineExceeded) {
			return nil, &TargetPluginError{Code: CodeTargetTimeout, Message: fmt.Sprintf("target plugin %s: timed out", p.path)}
		}
		return nil, &TargetPluginError{Code: CodeTargetProtocolError, Message: fmt.Sprintf("target plugin %s: %s", p.path, err)}
	}
	var resp targetPluginResponse
	if err := json.Unmarshal(out, &resp); err != nil {
		return nil, &TargetPluginError{Code: CodeTargetProtocolError, Message: fmt.Sprintf("target plugin %s: invalid JSON response: %s", p.path, err)}
	}
	return &resp, nil
}
