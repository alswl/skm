...
func TestNormalizeMovesSkillMissingProviderLevel(t *testing.T) {
	...
	dest, err := New(root).Normalize(context.Background(), entry, "", LifecycleOptions{})
	require.NoError(t, err)
...
func TestNormalizeDryRunWritesNothing(t *testing.T) {
	...
	dest, err := New(root).Normalize(context.Background(), entry, "", LifecycleOptions{DryRun: true})
	require.NoError(t, err)
	require.Equal(t, filepath.Join(root, "skills/local/flat-skill"), dest)
...
func TestNormalizeRefusesDestinationConflict(t *testing.T) {
	...
	_, err := New(root).Normalize(context.Background(), nonStandard, "", LifecycleOptions{})
	require.Error(t, err)
...
func TestNormalizeRejectsNonNonStandardEntry(t *testing.T) {
	...
	_, err := New(root).Normalize(context.Background(), entry, "", LifecycleOptions{})
	require.Error(t, err)
}
...
func TestNormalizeMovesToExplicitProvider(t *testing.T) {
	...
	dest, err := New(root).Normalize(context.Background(), entry, "github", LifecycleOptions{})
	require.NoError(t, err)
