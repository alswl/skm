...
func TestImportLocalDirPlacesUnderLocalLayerNoOrigin(t *testing.T) {
	...
	res, err := New(root).ImportStaged(context.Background(), src, "local", false, nil)
...
func TestImportCollisionRejectedWithoutForce(t *testing.T) {
	...
	_, err := New(root).ImportStaged(context.Background(), src, "local", false, nil)
...
func TestImportForceOverwriteAndRollback(t *testing.T) {
	...
	res, err := New(root).ImportStaged(context.Background(), src, "local", true, nil)
	require.NoError(t, err)
